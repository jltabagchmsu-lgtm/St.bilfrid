...
class WindowsDoorsTransferController extends Controller
{
    /**
     * Windows & Doors keywords and categories to strictly identify relevant items.
     */
    protected array $windowDoorKeywords = [
        'door', 'window', 'jamb', 'lockset', 'hinge', 'glass', 'analoc',
        'sliding', 'awning', 'pvc door', 'panel door', 'flush door', 'deadbolt',
        'casement', 'glazing', 'curtain wall'
    ];

    /**
     * Helper query to filter Windows & Doors materials.
     */
    protected function getWindowsDoorsMaterialsQuery()
    {
        return Material::where(function ($q) {
            $q->where('category', 'Windows & Doors')
              ->orWhere('category', 'Doors & Windows')
              ->orWhere(function ($sub) {
                  foreach ($this->windowDoorKeywords as $keyword) {
                      $sub->orWhere('name', 'like', "%{$keyword}%")
                          ->orWhere('material_code', 'like', "%DOOR%")
                          ->orWhere('material_code', 'like', "%WIN%");
                  }
              });
        });
    }
    ...
    /**
     * Display the Windows & Doors Material Transfer Station.
     */
    public function index(Request $request)
    {
        ...
        // Available warehouse windows & doors materials
        $materialsQuery = $this->getWindowsDoorsMaterialsQuery();
        if ($search) {
        ...
        $windowDoorMaterials = $materialsQuery->orderBy('name')->get();
        $windowDoorMaterialIds = $windowDoorMaterials->pluck('id')->toArray();
        ...
        // Active project site allocations for windows & doors
        $siteBOMQuery = ProjectMaterial::with(['project', 'material'])
            ->whereIn('material_id', $windowDoorMaterialIds);
        
        if ($selectedProjectId) {
            $siteBOMQuery->where('project_id', $selectedProjectId);
        }
        $projectWindowDoorMaterials = $siteBOMQuery->get();

        // Recent Windows & Doors Transfers
        $transfersQuery = ProjectMaterialTransfer::with(['sourceProject', 'destinationProject', 'material'])
            ->where(function ($q) use ($windowDoorMaterialIds) {
                $q->whereIn('material_id', $windowDoorMaterialIds)
                  ->orWhere('transfer_reference_no', 'like', 'XFER-WNDR-%');
            });
        ...
        // Stats
        $totalWarehouseStockUnits = $windowDoorMaterials->sum('stock_quantity');
        $totalWarehouseValuation = $windowDoorMaterials->sum(function ($m) {
            return $m->stock_quantity * $m->unit_cost;
        });
        $totalAllocatedUnits = $projectWindowDoorMaterials->sum('allocated_qty');
        $totalUsedUnits = $projectWindowDoorMaterials->sum('used_qty');
        $totalRemainingOnSite = $projectWindowDoorMaterials->sum(function ($pm) {
            return $pm->remaining_qty;
        });

        return view('transfer_portal.windows_doors.index', compact(
            'windowDoorMaterials',
            'projects',
            'selectedProject',
            'selectedProjectId',
            'projectWindowDoorMaterials',
            'recentTransfers',
            'search',
    ...
    /**
     * Dispatch windows & doors materials directly from Central Warehouse stock to a project site.
     */
     public function dispatchFromStock(Request $request)
     {
         if (Auth::user()->isAdmin()) {
             return redirect()->back()->with('error', 'Administrator accounts have View-Only auditing permissions for trade transfer portals. Changes must be executed by the Windows & Doors Transfer Officer.');
         }
         ...
         $transferRef = 'XFER-WNDR-' . date('Ymd') . '-' . strtoupper(substr(uniqid(), -4));
         $transfer = ProjectMaterialTransfer::create([
             'transfer_reference_no' => $transferRef,
             'source_project_id' => $destProject->id,
             'destination_project_id' => $destProject->id,
             'material_id' => $material->id,
             'quantity_transferred' => $qtyToTransfer,
             'transfer_date' => $validated['transfer_date'],
             'transfer_type' => 'warehouse_dispatch',
             'reason' => $validated['reason'] ?? 'Doors & windows fixtures dispatched from warehouse to project site.',
             'authorized_by' => Auth::user()->name ?? 'Windows & Doors Transfer Officer',
         ]);
         ...
         InventoryLog::create([
             ...
             'notes' => 'Windows & Doors dispatch: ' . number_format($qtyToTransfer) . ' ' . $material->unit . ' of ' . $material->name . ' to ' . $destProject->project_code . ' (' . $destProject->title . ').',
         ]);
     ...
     /**
      * Transfer surplus windows & doors materials between two active projects.
      */
     public function transferInterProject(Request $request)
     {
         ...
         ProjectMaterialTransfer::create([
             ...
             'reason' => $validated['reason'] ?? 'Inter-project door & window fixture balance transfer.',
             'authorized_by' => Auth::user()->name ?? 'Windows & Doors Transfer Officer',
         ]);
     ...
     /**
      * Return unused site door & window items back into Central Warehouse stock.
      */
     public function returnExcessToStock(Request $request)
     {
         ...
         $transferRef = 'RET-WNDR-' . date('Ymd') . '-' . strtoupper(substr(uniqid(), -4));
         ...
         ProjectMaterialTransfer::create([
             ...
             'reason' => $validated['reason'] ?? 'Surplus door / window fixtures reclaimed back into central warehouse inventory.',
             'authorized_by' => Auth::user()->name ?? 'Windows & Doors Transfer Officer',
         ]);
     ...
     /**
      * Restock windows & doors materials into Central Warehouse stock (Low Stock replenishment).
      */
     public function restockStock(Request $request)
     {
         ...
         $refNo = 'PO-WNDR-' . date('Ymd') . '-' . strtoupper(substr(uniqid(), -4));
         ...
         InventoryLog::create([
             ...
             'unit_cost' => $material->unit_cost,
             'reference_no' => $refNo,
             'notes' => 'Windows & Doors Restock: +' . number_format($restockQty) . ' ' . $material->unit . ' of ' . $material->name . ($validated['supplier_name'] ? ' (Supplier: ' . $validated['supplier_name'] . ')' : '') . ($validated['notes'] ? ' - ' . $validated['notes'] : ''),
         ]);
     ...
     public function printTransferVoucher($id)
     {
         ...
         $departmentName = 'Windows & Doors Division';
         $departmentOfficer = 'Windows & Doors Materials Transfer Officer';
