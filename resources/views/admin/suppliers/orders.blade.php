    ...
    <form method="GET" action="{{ route('admin.suppliers.orders') }}" style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 14px;">
        ...
        <div style="display: flex; align-items: center; gap: 10px; flex: 1; min-width: 260px;">
            <div style="position: relative; width: 100%; max-width: 360px;">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search Supplier Name, Order Code, Material, Site..." class="input-field search-input-with-icon" style="width: 100%; padding-left: 38px !important; font-size: 0.85rem;">
    ...
    <table class="grid-table">
        ...
        <tbody>
            @forelse($orders as $ord)
                ...
                <tr>
                    <td>
                        <strong style="font-family: var(--font-mono); color: var(--primary-red); font-size: 0.85rem;">{{ $ord->order_code }}</strong>
                    ...
                    <td>
                        <span style="font-size: 0.8rem; font-family: var(--font-mono); font-weight: 700; color: var(--primary-red);">
                            {{ $ord->requested_delivery_date->format('M d, Y') }}
                        </span>
                    </td>
                    <td>
                        <span style="font-size: 0.8rem; font-family: var(--font-mono); color: {{ $ord->actual_delivery_date ? '#059669' : 'var(--text-muted)' }};">
                        ...
                        <div style="display: flex; align-items: center; gap: 6px;">
                            ...
                            @if($ord->status === 'delivered')
                                <form action="{{ route('admin.suppliers.orders.receive', $ord->id) }}" method="POST" onsubmit="return confirm('Officially accept delivery and synchronize materials into Project BOM?');">
                                    @csrf
                                    <button type="submit" class="btn-primary" style="padding: 5px 10px; font-size: 0.75rem; background: #059669; border-color: #059669;" title="Accept materials into BOM stock">
                                        Accept & BOM Sync
                                    </button>
                                </form>
                            @endif
    ...
    <div class="modal-box" style="max-width: 760px;">
        <div class="modal-header">
            <div>
                ...
                <p id="adminModalOrderCodeHeader" style="font-size: 0.8rem; color: var(--primary-red); font-family: var(--font-mono); margin-top: 2px;"></p>
            </div>
        ...
        <div class="modal-body" style="padding-top: 18px;">
            ...
            <div style="background: var(--bg-surface-alt); border: 1px solid var(--border-color); border-radius: 12px; padding: 16px; margin-bottom: 20px; display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px;">
                <div>
                ...
                <div>
                    ...
                    <div id="adminModalRequestedDelivery" style="font-size: 0.85rem; font-weight: 700; color: var(--primary-red); font-family: var(--font-mono); margin-top: 4px;"></div>
                </div>
                <div>
            ...
            <div style="margin-bottom: 20px; font-size: 0.85rem; background: var(--bg-surface-alt); border: 1px solid var(--border-color); border-radius: 10px; padding: 14px;">
                ...
                <div id="adminModalNotesContainer" style="display: none;">
                    ...
                    <div id="adminModalNotes" style="color: var(--text-secondary); margin-top: 4px; font-style: italic; background: #fafbfc; border: 1px solid var(--border-color); padding: 8px 12px; border-radius: 6px;"></div>
                </div>
            </div>
            ...
            <div class="order-chat-container">
                <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px;">
                    <h4 style="font-size: 0.85rem; font-weight: 700; color: var(--primary-red); display: flex; align-items: center; gap: 8px;">
            ...
            <form id="adminUpdateOrderStatusForm" method="POST" style="background: var(--bg-surface-alt); border: 1px solid var(--border-color); border-radius: 12px; padding: 18px; margin-bottom: 20px;">
                @csrf
                <h4 style="font-size: 0.9rem; font-weight: 700; color: var(--primary-red); margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
            ...
            <div>
                ...
                <div id="adminModalStatusLogs" style="background: var(--bg-surface-alt); border: 1px solid var(--border-color); border-radius: 10px; padding: 14px; max-height: 180px; overflow-y: auto;">
                    <!-- Populated dynamically -->
                </div>
            </div>
        </div>
    ...
    function openAdminOrderModal(order) {
        ...
        if (order.items && order.items.length > 0) {
            order.items.forEach(function(item) {
                ...
                tr.innerHTML = `
                    ...
                    <td style="text-align: right; font-family: var(--font-mono); font-weight: 700; color: var(--primary-red);">PHP ${Number(item.total_price).toLocaleString('en-US', {minimumFractionDigits: 2})}</td>
                `;
        ...
        if (order.logs && order.logs.length > 0) {
            order.logs.forEach(function(log) {
                ...
                logItem.style.cssText = 'padding: 8px 0; border-bottom: 1px solid var(--border-color); font-size: 0.8rem; display: flex; align-items: center; justify-content: space-between;';
                ...
                logItem.innerHTML = `
                    <div>
                        <span style="font-weight: 700; color: var(--primary-red);">${log.to_status.toUpperCase().replace(/_/g, ' ')}</span>: 
                    ...
                    data.messages.forEach(msg => {
                        ...
                        bubble.innerHTML = `
                            <div class="chat-msg-header">
                                <span class="chat-msg-sender" style="color: ${isAdm ? 'var(--primary-red)' : '#059669'};">
